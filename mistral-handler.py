# mistral_handler.py - v1.5.0
"""
Handler pour API Mistral AI avec nettoyage email InReach
Compatible avec architecture modulaire email_monitor v3.2.3

v1.5.0:
- NETTOYAGE EMAIL INREACH avant envoi à Mistral
...
def handle_mistral_maritime_assistant(raw_email_body: str) -> Tuple[str, float]:
    """
    Assistant maritime spécialisé avec Mistral + NETTOYAGE EMAIL
    ...
    try:
        print(f"\n{'='*70}")
        print("🧠 MISTRAL MARITIME ASSISTANT v1.5.0")
        print(f"{'='*70}")
...
def handle_mistral_request(raw_email_body: str, max_tokens: int = 1024) -> Tuple[str, float]:
    """
    Requête Mistral générique (non spécialisée maritime) + NETTOYAGE EMAIL
...
def handle_mistral_weather_expert(raw_email_body: str) -> Tuple[str, float]:
    """
    Expert météo marine spécialisé avec Mistral + NETTOYAGE EMAIL
    ...
    try:
        print(f"\n{'='*70}")
        print("🌊 MISTRAL WEATHER EXPERT v1.5.0")
        print(f"{'='*70}")
        print(f"📧 Email nettoyé ({len(user_message)} chars)")
        print(f"Question: {user_message[:100]}...")

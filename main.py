...
def query_claude(prompt, max_words=50):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Découper la réponse
            messages = split_into_messages(answer, max_chars_per_message=92)  # Réduire pour [C X/Y]
            ...
            print(f"📨 Réponse Claude découpée en {total_msgs} message(s)")
            for i, msg in enumerate(numbered_messages, 1):
                print(f"   [C {i}/{total_msgs}]: {len(msg)} chars - '{msg[:60]}...'")
...
def query_mistral(prompt, max_words=50):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Découper la réponse
            messages = split_into_messages(answer, max_chars_per_message=92)  # Réduire pour [M X/Y]
            ...
            print(f"📨 Réponse Mistral découpée en {total_msgs} message(s)")
            for i, msg in enumerate(numbered_messages, 1):
                print(f"   [M {i}/{total_msgs}]: {len(msg)} chars - '{msg[:60]}...'")

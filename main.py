...
# CHANGELOG:
#  2.2.0 (2025-12-15): Playwright pour automatiser formulaire Garmin web
...
# Playwright pour automatisation navigateur
try:
    from playwright.sync_api import sync_playwright, TimeoutError as PlaywrightTimeout
    PLAYWRIGHT_AVAILABLE = True
    print("? Playwright disponible - mode navigateur active", flush=True)
except ImportError:
    PLAYWRIGHT_AVAILABLE = False
    print("? Playwright non disponible - fallback API POST", flush=True)
...
def send_messages_via_playwright(inreach_url, messages):
    """
    Envoie des messages via automatisation du navigateur Garmin
    Utilise Playwright pour remplir le formulaire web
    """
    if not PLAYWRIGHT_AVAILABLE:
        print("? Playwright non disponible, impossible d'utiliser le navigateur", flush=True)
        return False
    
    print(f"? Envoi de {len(messages)} messages via navigateur automatise", flush=True)
    ...
    try:
        with sync_playwright() as p:
            ...
            print("? Lancement navigateur Chromium...", flush=True)
            ...
            try:
                for i, message in enumerate(messages, 1):
                    ...
                    try:
                        # Navigation
                        print(f"  ? Navigation vers formulaire...", flush=True)
                        page.goto(inreach_url, timeout=30000)
                        
                        # Attendre chargement formulaire
                        page.wait_for_selector('textarea[name="ReplyMessage"], textarea#ReplyMessage', timeout=10000)
                        
                        # Remplir message
                        print(f"  ? Remplissage: {message[:50]}...", flush=True)
                        page.fill('textarea[name="ReplyMessage"], textarea#ReplyMessage', message)
                        
                        # Attendre un peu
                        page.wait_for_timeout(1000)
                        
                        # Cliquer envoi
                        print(f"  ? Clic Envoyer...", flush=True)
                        page.click('button[type="submit"], input[type="submit"], button:has-text("Send")')
                        ...
                        if "error" in content or "failed" in content:
                            print(f"  ? Message {i}: Erreur detectee", flush=True)
                        else:
                            print(f"  ? Message {i}: Envoye", flush=True)
                            success_count += 1
                        
                    except PlaywrightTimeout as e:
                        print(f"  ? Timeout message {i}: {e}", flush=True)
                    except Exception as e:
                        print(f"  ? Erreur message {i}: {e}", flush=True)
                    finally:
                        page.close()
                    
                    # Delai entre messages
                    if i < len(messages):
                        time.sleep(3)
        ...
        print(f"? Erreur navigateur: {e}", flush=True)
        import traceback
        traceback.print_exc()
        return False
    
    print(f"\n? {success_count}/{len(messages)} messages envoyes via navigateur", flush=True)
    return success_count == len(messages)

def send_messages_to_inreach(url, messages):
    """
    Envoie les messages vers inReach
    Priorite: Playwright (navigateur) > API POST (deprecie)
    """
    
    print(f"? Envoi de {len(messages)} messages vers inReach", flush=True)
    
    # PRIORITE 1: Utiliser Playwright si disponible
    if PLAYWRIGHT_AVAILABLE:
        print("? Utilisation navigateur automatise (Playwright)", flush=True)
        return send_messages_via_playwright(url, messages)
    
    # FALLBACK: Essayer API POST (peut ne plus fonctionner)
    print("? Fallback: Tentative API POST (peut echouer)", flush=True)
    
    try:
        guid = extract_guid_from_url(url)
        print(f"? GUID extrait: {guid}", flush=True)
        
        # RECONSTRUCTION de l'URL complete avec le parametre adr manquant
        # L'ancienne URL avait : ?extId=XXX&adr=email
        # La nouvelle n'a que : ?extId=XXX
        # On doit RECONSTRUIRE l'URL complete
        
        # Encoder l'email pour URL (@ devient %40)
        ...
        print(f"? Email encode: {email_encoded}", flush=True)
        print(f"? URL reconstruite: {post_url}", flush=True)
    ...
    for i, message in enumerate(messages, 1):
        try:
            ...
            response = requests.post(
                post_url,  # URL reconstruite avec adr

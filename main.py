...
# CHANGELOG:
#  2.1.1 (2025-12-15): Ajout debug email encoding + fix URL reconstruction
#  2.1.0 (2025-12-15): Fix API Garmin - reconstruction URL avec parametre adr
...
def send_messages_to_inreach(url, messages):
    """Envoie les messages vers inReach via POST - Support 2 formats URL"""
    
    print(f"? Envoi de {len(messages)} messages vers inReach", flush=True)
    print(f"? URL recue: {url}", flush=True)
    
    try:
        # ETAPE 1: Detecter le type d'URL
        if 'inreachlink.com' in url.lower():
            print(f"? Format detecte: NOUVEAU (inreachlink.com - Messenger)", flush=True)
            # Suivre la redirection pour obtenir l'URL complete
            print(f"? Suivi de la redirection...", flush=True)
            response = requests.get(url, allow_redirects=True, timeout=10)
            final_url = response.url
            print(f"? URL finale apres redirection: {final_url}", flush=True)
            
            # Extraire extId et adr de l'URL finale
            parsed = urlparse(final_url)
            params = parse_qs(parsed.query)
            guid = params.get('extId', [None])[0]
            adr_param = params.get('adr', [None])[0]
            
            if not guid:
                print(f"? ERREUR: Impossible d'extraire extId de l'URL redirigee", flush=True)
                return False
            
            print(f"? extId extrait: {guid}", flush=True)
            if adr_param:
                print(f"? adr extrait: {adr_param}", flush=True)
            
            # Construire l'URL POST
            email_encoded = GARMIN_USERNAME.replace('@', '%40')
            post_url = f"https://eur.explore.garmin.com/textmessage/txtmsg?extId={guid}&adr={email_encoded}"
            
        elif 'garmin.com' in url.lower():
            print(f"? Format detecte: ANCIEN (explore.garmin.com - Earthmate)", flush=True)
            # URL directe format ancien
            parsed = urlparse(url)
            params = parse_qs(parsed.query)
            guid = params.get('extId', [None])[0]
            adr_param = params.get('adr', [None])[0]
            ...
            if not guid:
                print(f"? ERREUR: Impossible d'extraire extId de l'URL", flush=True)
                return False
            
            print(f"? extId extrait: {guid}", flush=True)
            
            # Verifier si adr est present, sinon l'ajouter
            if adr_param:
                print(f"? adr deja present: {adr_param}", flush=True)
                post_url = url
            else:
                print(f"? adr manquant, ajout...", flush=True)
                email_encoded = GARMIN_USERNAME.replace('@', '%40')
                post_url = f"https://eur.explore.garmin.com/textmessage/txtmsg?extId={guid}&adr={email_encoded}"
        else:
            print(f"? ERREUR: URL inconnue (ni inreachlink ni garmin)", flush=True)
            return False
        
        print(f"? URL POST finale: {post_url}", flush=True)
        
    except Exception as e:
        print(f"? Erreur traitement URL: {e}", flush=True)
        import traceback
        traceback.print_exc()
        return False
    ...
    for i, message in enumerate(messages, 1):
        try:
            ...
            if response.status_code == 200:
                print(f"? Message {i}/{len(messages)} envoye (Status: {response.status_code})", flush=True)
                success_count += 1
            elif response.status_code == 500:
                print(f"? Message {i}/{len(messages)} - Status: 500 (Erreur serveur Garmin)", flush=True)
                error_500_count += 1
            else:
                print(f"? Message {i}/{len(messages)} - Status: {response.status_code}", flush=True)
                print(f"  Reponse: {response.text[:200]}", flush=True)
            
            if i < len(messages):
                time.sleep(DELAY_BETWEEN_MESSAGES)
                
        except Exception as e:
            print(f"? Erreur message {i}/{len(messages)}: {e}", flush=True)
    
    # Si tous les messages ont Status 500, l'API POST ne fonctionne pas
    if error_500_count == len(messages):
        print(f"\n? TOUS LES MESSAGES ONT Status 500 - API POST Garmin ne fonctionne pas", flush=True)
        return False
    
    if success_count == len(messages):
        print(f"\n? TOUS LES {len(messages)} MESSAGES ENVOYES!", flush=True)
        return True
    else:
        print(f"\n? {success_count}/{len(messages)} messages envoyes", flush=True)
        return False
    ...
    try:
        ...
        for idx, req in enumerate(requests_list, 1):
            ...
            if req['type'] == 'ai':
                ...
                # Envoyer tous les messages via POST Garmin (methode originale)
                # Si echec (status 500), utiliser email comme fallback
                post_success = send_messages_to_inreach(req['reply_url'], messages)
                
                if post_success:
                    print(f"??? SUCC?S COMPLET {provider.upper()}: {len(messages)} messages envoyes via POST ???")
                    last_status = f"? {provider.upper()}: {len(messages)} msg OK (POST)"
                else:
                    print(f"\n{'='*70}")
                    print(f"? POST GARMIN ECHEC - ESSAI FALLBACK EMAIL")
                    print(f"{'='*70}\n")
                    
                    # Fallback: envoyer par email
                    if send_messages_to_inreach_via_email(req['reply_email'], messages):
                        print(f"??? SUCC?S {provider.upper()} via EMAIL: {len(messages)} messages envoyes ???")
                        last_status = f"? {provider.upper()}: {len(messages)} msg OK (EMAIL)"
                    else:
                        print(f"?????? ECHEC COMPLET {provider.upper()} ??????")
                        last_status = f"? {provider.upper()} echec total"
            ...
            # TRAITEMENT GRIB
                ...
                # Essayer POST d'abord, puis email en fallback
                post_success = send_messages_to_inreach(req['reply_url'], messages)
                
                if post_success:
                    print(f"??? GRIB ENVOYE AVEC SUCC?S via POST ({len(messages)} msg) ???", flush=True)
                    last_status = f"? GRIB envoye ({len(messages)} msg POST)"
                else:
                    print(f"\n? POST GARMIN ECHEC - ESSAI FALLBACK EMAIL\n", flush=True)
                    
                    if send_messages_to_inreach_via_email(req['reply_email'], messages):
                        print(f"??? GRIB ENVOYE AVEC SUCC?S via EMAIL ({len(messages)} msg) ???", flush=True)
                        last_status = f"? GRIB envoye ({len(messages)} msg EMAIL)"
                    else:
                        print(f"? Echec envoi GRIB total", flush=True)
                        last_status = f"? Echec GRIB total"

...
def query_claude(prompt, max_words=50):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # InReach limite = 160 chars, on prend 110 pour la marge [M X/Y] + coût
            messages = split_into_messages(answer, max_chars_per_message=100)
...
def query_mistral(prompt, max_words=50):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Découper la réponse avec VRAIE limite InReach
            # InReach limite = 160 chars, on prend 100 pour la marge [M X/Y] + coût
            messages = split_into_messages(answer, max_chars_per_message=100)
...
def send_message_to_inreach(url, message):
    ...
    # SÉCURITÉ CRITIQUE: Garmin InReach limite à 160 chars
    # On garantit 120 chars max pour être sûr
    if len(message) > 120:
        print(f"      ⚠️  MESSAGE TROP LONG: {len(message)} chars! Truncation à 120...", flush=True)
        message = message[:117] + "..."

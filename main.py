        ...
        for email_id in email_ids:
            ...
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    ...
                    # Format 1: https://inreachlink.com/GUID (avec _, -, lettres, chiffres)
                    # Format 2: https://...garmin.com/textmessage/txtmsg?...
                    # Note: GUID peut contenir: lettres, chiffres, tirets (-), underscores (_)
                    reply_url_pattern = re.compile(
                        r'https://[^\s<>"\']+(?:inreachlink\.com|garmin\.com)[^\s<>"\']*',
                        re.IGNORECASE
                    )
                    ...
                    if not reply_match:
                        ...
                        # Afficher en morceaux pour voir toutes les parties
                        for i, part_text in enumerate(body_parts, 1):
                            print(f"   --- Partie {i}/{len(body_parts)} ({len(part_text)} chars) ---", flush=True)
                            print(part_text[:500], flush=True)
                        continue
                    
                    reply_url = reply_match.group(0).strip()
                    # Nettoyer les caractères HTML éventuels à la fin
                    reply_url = reply_url.rstrip('.,;)\'"<>')
                    print(f"✅ URL de réponse trouvée: {reply_url}", flush=True)
...
def send_message_to_inreach(url, message):
    ...
    try:
        ...
        print(f"      🔧 Envoi POST vers Garmin...", flush=True)

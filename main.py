        ...
        for email_id in email_ids:
            ...
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    ...
                    # IMPORTANT: Extraire aussi l'adresse email de réponse
...
def send_messages_to_inreach_via_email(reply_email, messages):
    """
    Envoie des messages vers inReach en répondant par EMAIL
    C'est la méthode officielle supportée par Garmin!
    """
    ...
    sendgrid_api_key = os.environ.get('SENDGRID_API_KEY')
    if not sendgrid_api_key:
        print("❌ SENDGRID_API_KEY non définie", flush=True)
        return False
    ...
    for i, message in enumerate(messages, 1):
        try:
            ...
            if response.status_code == 202:  # SendGrid retourne 202 Accepted
                print(f"   ✅ Message {i}/{len(messages)} envoyé - Status: 202", flush=True)
                success_count += 1
            else:
    ...
    if success_count == len(messages):
        print(f"✅ Tous les messages envoyés par email avec succès!", flush=True)
    else:
        print(f"⚠ {success_count}/{len(messages)} messages envoyés", flush=True)
    
    return success_count == len(messages)

def send_messages_to_inreach(url, messages):
    """Envoie les messages vers inReach via POST vers l'API fixe"""
    ...
    # URL FIXE de l'API (pas l'URL de l'email !)
    api_url = "https://eur.explore.garmin.com/TextMessage/TxtMsg"
    ...
    for i, message in enumerate(messages, 1):
        try:
            ...
            response = requests.post(
                api_url,  # URL fixe, pas l'URL de l'email
            ...
            if response.status_code == 200:
                print(f"✅ Message {i}/{len(messages)} envoyé (Status: {response.status_code})", flush=True)
                success_count += 1
            else:
                print(f"⚠ Message {i}/{len(messages)} - Status: {response.status_code}", flush=True)
    ...
    if success_count == len(messages):
        print(f"\n✅ TOUS LES {len(messages)} MESSAGES ENVOYÉS!", flush=True)
        return True
    else:
        print(f"\n⚠ {success_count}/{len(messages)} messages envoyés", flush=True)
        return False
    ...
    try:
        ...
        for idx, req in enumerate(requests_list, 1):
            ...
            if req['type'] == 'ai':
                ...
                # Envoyer tous les messages via POST Garmin (méthode originale)
                if send_messages_to_inreach(req['reply_url'], messages):
                    print(f"✅✅✅ SUCCÈS COMPLET {provider.upper()}: {len(messages)} messages envoyés ✅✅✅")
                    last_status = f"✅ {provider.upper()}: {len(messages)} msg OK"
                else:
                    print(f"⚠️⚠️⚠️ ÉCHEC {provider.upper()} ⚠️⚠️⚠️")

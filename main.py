...
def query_claude(prompt, max_words=50):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Découper la réponse
            messages = split_into_messages(answer, max_chars_per_message=95)  # Réduire pour faire place à la numérotation
            
            # Ajouter numérotation et coût
            ...
            for i, msg in enumerate(messages, 1):
                if i == total_msgs:
                    ...
                    numbered_msg = f"[{i}/{total_msgs}] {msg} [${total_cost:.4f}]"
                else:
                    numbered_msg = f"[{i}/{total_msgs}] {msg}"
                numbered_messages.append(numbered_msg)
            
            print(f"📨 Réponse découpée en {total_msgs} message(s)")
            for i, msg in enumerate(numbered_messages, 1):
                print(f"   Message {i}/{total_msgs}: {len(msg)} chars")
    ...
    if len(messages) > 4:
        ...
        for i in range(4):
            ...
            if start < len(text):
                ...
                if i == 3 and end < len(text):
                    msg += "..."
                messages.append(msg)
    
    # Vérification finale
    if not messages:
        print("⚠️  ATTENTION: split_into_messages a généré 0 messages!")
        messages = [text[:max_chars_per_message]]
    
    print(f"   🔍 Découpage: {len(text)} chars → {len(messages)} message(s)")
    for i, msg in enumerate(messages, 1):
        print(f"      Msg {i}: {len(msg)} chars - Début: '{msg[:40]}...'")
...
def send_message_to_inreach(url, message):
    ...
    try:
        ...
        print(f"      🔧 Envoi POST vers Garmin...")
        ...
        if response.status_code == 200:
            print(f"      ✅ Status 200 OK - Message accepté par Garmin")
            return True
        else:
            print(f"      ❌ Status {response.status_code} - Erreur Garmin")
    ...
    try:
        ...
        for req in requests_list:
            ...
            if req['type'] == 'ai':
                ...
                print(f"\n📦 TOTAL DE {len(messages)} MESSAGES À ENVOYER")
                print("="*60)
                
                # Envoyer les messages avec vérification
                success_count = 0
                for i, message in enumerate(messages, 1):
                    print(f"\n📤 Tentative d'envoi message {i}/{len(messages)}...")
                    print(f"   Contenu ({len(message)} chars): {message[:80]}...")
                    ...
                    if not sent:
                        print(f"   ❌ Message {i}/{len(messages)} ÉCHEC après 3 tentatives")
                        break
                ...
                if success_count == len(messages):
                    print(f"✅ SUCCÈS COMPLET : {success_count}/{len(messages)} messages envoyés")
                    last_status = f"✅ {provider.upper()}: {success_count}/{len(messages)} msg OK"
                    print(f"\n✅ REQUÊTE {provider.upper()} TRAITÉE!\n")
                else:
                    print(f"⚠️  SUCCÈS PARTIEL : {success_count}/{len(messages)} messages envoyés")
                    last_status = f"⚠️  {provider.upper()}: {success_count}/{len(messages)} msg"
                    print(f"\n⚠️  Échec partiel - {success_count}/{len(messages)} envoyés\n")
                print("="*60)

        ...
        for email_id in email_ids:
            ...
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    ...
                    reply_url_pattern = re.compile(r'https://[^\s]+(inreachlink\.com|garmin\.com)[^\s]+')
                    reply_match = reply_url_pattern.search(body)
                    
                    if not reply_match:
                        print("⚠️ Email sans URL de réponse valide", flush=True)
                        print(f"   Body complet: {body}", flush=True)
                        continue
                    
                    reply_url = reply_match.group(0).strip()
                    print(f"✅ URL de réponse trouvée: {reply_url[:60]}...", flush=True)

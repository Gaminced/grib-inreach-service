    ...
    with sync_playwright() as p:
        try:
            ...
            for i, message in enumerate(messages, 1):
                try:
                    print(f"\n   📤 Message {i}/{len(messages)}: {len(message)} chars", flush=True)
                    
                    # CRITIQUE: Attendre plus longtemps entre messages
                    # et ne PAS attendre networkidle (trop long)
                    if i > 1:
                        print(f"      ⏸️  Pause {DELAY_BETWEEN_MESSAGES + 3}s...", flush=True)
                        time.sleep(DELAY_BETWEEN_MESSAGES + 3)  # 8 secondes au lieu de 5
                    
                    # Re-localiser textarea à CHAQUE fois
                    print(f"      🔍 Localisation textarea...", flush=True)
                    textarea = page.locator('textarea').first
                    
                    # Attendre que textarea soit visible ET éditable
                    textarea.wait_for(state='visible', timeout=15000)
                    time.sleep(1)
                    
                    # Vider textarea
                    print(f"      🧹 Nettoyage textarea...", flush=True)
                    textarea.fill('')
                    time.sleep(0.5)
                    
                    # Remplir message
                    print(f"      ✍️  Remplissage message...", flush=True)
                    textarea.fill(message)
                    time.sleep(1)
                    
                    # Cliquer Send
                    print(f"      📮 Clic Send...", flush=True)
                    send_button = page.locator('button:has-text("Send"), input[type="submit"]').first
                    send_button.click()
                    
                    # Attendre que le message soit parti (textarea se vide)
                    time.sleep(2)

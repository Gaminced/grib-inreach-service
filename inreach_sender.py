...
def send_via_playwright_inreachlink(url, messages):
    ...
    with sync_playwright() as p:
        try:
            ...
            page.wait_for_selector('textarea[name="ReplyMessage"]', timeout=20000)
            time.sleep(2)
            ...
            for i, message in enumerate(messages, 1):
                try:
                    ...
                    # Re-localiser le textarea pour chaque message (élément peut être recréé)
                    textarea = page.locator('textarea[name="ReplyMessage"]')
                    
                    # Vérifier que le textarea est visible et enabled
                    if not textarea.is_visible():
                        print(f"      ⚠️  Textarea non visible, attente...")
                        page.wait_for_selector('textarea[name="ReplyMessage"]:visible', timeout=10000)
                        textarea = page.locator('textarea[name="ReplyMessage"]')
                    ...
                    # Vérifier succès (peut varier selon version Garmin)
